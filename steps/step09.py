...
class Variable:
    ...
    def backward(self):
        ...
        if self.grad is None:
            self.grad = np.ones_like(self.data) #만약 변수의 grad가 None이면 자동으로 미분값을 1로 생성한다.
...
문제: 0차원 ndarray 인스턴스를 사용하여 계산하면 결과의 데이터 타입이 numpy.float64나 numpy.float32 등이 된다.
    ...
    Dezeron 함수의 계산 결과(출력)이 numpy.float64나 numpy.float32가 되지 않도록 해야한다.
"""
def as_array(x):
...
class Function:
    def __call__(self, input):
        x = input.data
        y = self.forward(x)
        output = Variable(as_array(y))  #출력 결과인 output이 항상 ndarray 인스턴스가 되도록 보장한다.
